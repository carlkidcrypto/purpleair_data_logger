...
```bash
bash scripts/release.sh --dry-run 1.5.1 1.5.1a2
```

Apply the stable release bump:

```bash
bash scripts/release.sh 1.5.1 1.5.1a2
```
...
The script accepts stable versions such as `1.5.1` and prereleases such as `1.5.1a2`, `1.5.1b1`, and `1.5.1rc1`. The new version is supplied without a leading `v`; existing `v` prefixes in documentation are preserved.
...
For the `1.5.1a2` to `1.5.1` release:

1. Run `bash scripts/release.sh --dry-run 1.5.1 1.5.1a2` and review the selected files.
2. Run `bash scripts/release.sh 1.5.1 1.5.1a2`.
...
7. Create and push the release tag, for example `v1.5.1`.
