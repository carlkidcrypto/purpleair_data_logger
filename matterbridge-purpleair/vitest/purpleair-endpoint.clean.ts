...
import { createPurpleAirEndpoint, purpleAirSerialNumber, updatePurpleAirEndpoint } from '../src/purpleair-endpoint.js';
...
describe('PurpleAir Matter endpoint', () => {
  ...
  it('omits TVOC when the source does not provide it', () => {
    ...
    expect(endpoint.hasClusterServer(TotalVolatileOrganicCompoundsConcentrationMeasurement)).toBe(false);
  });
  ...
  it('updates existing cluster attributes in place', async () => {
    ...
    await updatePurpleAirEndpoint(endpoint, { ...reading, airQuality: 4, temperature: 2200, pm25: 40.5 });
