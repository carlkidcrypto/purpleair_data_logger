...
A ``matterbridge-purpleair`` plugin can provide the missing protocol boundary.
...
The plugin described on this page is a proposed companion component. It is not
yet included in this repository or published to npm.
...
.. list-table:: Required mapping
   ...
   * - ``device.device_type.id``
     ...
     - The current decimal value is ``45``.
...
Proposed companion project
--------------------------

The companion can be developed in a sibling directory at the repository root::
...
The recommended starting point is the official Matterbridge plugin template.
The plugin should use TypeScript, the Matterbridge plugin API, and the repository
versions of its formatter, linter, and test runner. Its configuration should
include at least:

``baseUrl``
   PurpleAir Matter logger URL. Default: ``http://127.0.0.1:9855``.
...
``sensorIndexes``
   Optional allowlist of PurpleAir sensor indexes. An empty list exposes every
   sensor returned by the logger.

``removeMissingSensors``
   Whether sensors absent for a configured grace period may be removed. This
   should default to ``false`` to preserve endpoint identity.
...
The plugin can then use ``http://127.0.0.1:9855``. If Matterbridge runs on a
...
After the companion plugin has been implemented and built, the expected local
development flow is::

   npm install
   npm run build
   npm test
   matterbridge --add ./matterbridge-purpleair
   matterbridge --bridge

Exact commands may change with the Matterbridge plugin template. Matterbridge
provides its frontend on port ``8283`` by default and displays the bridge QR code
in the frontend and console.
