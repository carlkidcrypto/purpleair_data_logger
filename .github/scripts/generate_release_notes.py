#!/usr/bin/env python3
"""Generate and update GitHub release notes for purpleair_data_logger.

This script automates the generation of high-signal, human-readable release
notes for tagged purpleair_data_logger releases, following the specification in
...
4. Generates a formatted release note matching the purpleair_data_logger template.
...
DEFAULT_REPO = "carlkidcrypto/purpleair_data_logger"
DEFAULT_PACKAGE = "purpleair_data_logger"
...
def categorize_item(title: str, files: list[str]) -> str:
    ...
    # 4. Tests
    if files and all(f.startswith(("tests/", "behave_tests/")) for f in files):
        return "Tests"
    ...
    if (
        ...
        and not any(f.startswith("purpleair_data_logger/") for f in files)
    ):
        return "Packaging"
    if any(
        title_lower.startswith(p)
    ...
    touches_core = any(f.startswith("purpleair_data_logger/") for f in files)
    is_bug = any(
        w in title_lower
        for w in [
            "fix",
...
def synthesize_summary(
    ...
    if categories["Data Loggers / Core"]:
        highlights.append("core data logger improvements")
    ...
    summary = f"This {release_type} focuses on {hl_str} across the purpleair_data_logger package."
...
def build_release_notes(
    ...
    """Build formatted release notes markdown adhering to the purpleair_data_logger template."""
    tag = release["tag_name"]
    is_prerelease = release.get("prerelease", False)
    pypi_version = tag[1:] if tag.startswith("v") else tag
    has_valid_pypi = bool(re.match(r"^\d+(\.\d+)*", pypi_version))
    pypi_url = f"https://pypi.org/project/purpleair_data_logger/{pypi_version}/"
    ...
    if has_valid_pypi:
        lines.extend(
            [
                ...
                "",
                "```bash",
                f"pip install purpleair_data_logger=={pypi_version}",
                "```",
                "",
